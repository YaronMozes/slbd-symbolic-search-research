...
In practice the search is over a coarser space: the optimizer permutes the $n$ $\text{SAS}^+$ variables, and each variable's $m_i$ bits -- interleaved with their primed counterparts -- occupy a contiguous block of the induced Boolean order. All objectives below are therefore stated over $\text{SAS}^+$ variable positions.
...
\begin{equation}
    ...
    \label{eq:image_comp}
\end{equation}
where $T(X, X') = \bigvee_{a \in \mathcal{A}} T_a(X, X')$ represents the system transition relation, and $C(X')$ is the invariant constraint characterising valid states, compiling all $h^2$ mutexes and exactly-one groups \cite{torralba2013constrained}:
\begin{equation}
\begin{aligned}
...
\end{equation}
where $\mathcal{M}_{i,j}$ is the set of invariant groups containing co-occurring facts for variables $v_i$ and $v_j$, and each group contributes once \emph{per co-occurring fact pair} (a group holding several facts of $v_i$ and $v_j$ adds $w_{\text{pair}}$ with the corresponding multiplicity). In the confirmed policy configuration this reduces to $w_{\text{pair}}(M) = w_{\text{co}}$ and $\text{SkipCausal} \equiv 0$.
...
\begin{tikzpicture}[node distance=0.85cm and 0.6cm, auto, >=Stealth]
    ...
    \node [draw, rectangle, rounded corners=3pt, fill=bglight, draw=primary, line width=0.8pt, left=0.5cm of order, text width=2.3cm, align=center] (symk) {\scriptsize \textsf{SymK} Search Engine\\ Image over\\ constraint-compiled TRs};
...
\label{sec:policy}
Because constraint density varies by domain structure, applying constraint ordering suite-wide induces domain-specific trade-offs. To evaluate the replicated per-domain signals under a fixed protocol, we define a targeted per-domain policy $\mathcal{W}(d)$ mapping each domain $d$ to its constraint edge weight:
\begin{equation}
...
\subsection{Per-Domain Gains Under a Pre-Registered A/B}
...
To validate this signal, we pre-registered a targeted per-domain policy: enabling constraint-aware variable ordering with per-domain weights $\mathcal{W}(d)$ (Equation~\ref{eq:policy_weights}) on the five signal domains while executing stock \textsf{SymK} elsewhere. We executed a confirmatory, double-replicated A/B study with 480 runs, placing each instance's stock and policy jobs adjacently in the execution queue so that the two arms meet similar machine load. Adjacency reduces but does not eliminate order effects: the queue always emits stock before policy, and we did not counterbalance.

As summarized in Table~\ref{tab:prereg_ab}, \textbf{all four pre-registered evaluation gates passed}. The per-domain coverage gains relative to stock \textsf{SymK} across both execution replicates are detailed in Table~\ref{tab:ab_domain_breakdown}.

\begin{funfact}[Headline Result: Repeated $+6$ Coverage on Five Post-Hoc-Selected Domains]
...
The overall coverage difference ($-6$ instances) is statistically unremarkable: discordant pairs show 13 gains versus 19 losses (exact sign test $p = 0.38$), and we have no clean full-suite replicate from which to calibrate how much coverage drift this protocol produces on its own (Section~\ref{sec:ceiling}).

On commonly solved instances ($n = 1{,}126$), the wall-clock geometric mean ratio is 1.005 with a median of 1.000 and a domain-clustered 95\% confidence interval of [0.97, 1.05] (Table~\ref{tab:suitewide_results}). This bounds the always-on ordering's mean wall-clock effect on commonly solved instances to within about 5\%; it does not bound coverage, which is addressed above.
...
runtime ($\rho = 0.199$) -- a diffuse cloud around a nearly flat trend.
...
% ==================== TABLE 1 ====================
\caption{Pre-registered confirmatory A/B evaluation results (480 total runs across 2 execution replicates).}
\label{tab:prereg_ab}
\small
\begin{tabular}{llcc}
\toprule
...
Integrity: Cost Agreement & No cost disagreement & \textbf{0 / 129 comparable pairs} & \textbf{PASSED} \\
\bottomrule
\end{tabular}
...
Three limitations bound the claims above. First, and most important, \textbf{the five signal domains were selected post hoc} from the same full-suite runs that produced the suite-wide analysis. The pre-registered A/B establishes that the effect on those domains is \emph{reproducible} rather than run-to-run noise: the aggregate $+6$ recurs in both interleaved replicates with no negative per-domain cell, though the instance-level pattern varies (4 of the 8 distinct newly solved instances gain in both replicates). It does not make $+6$ an out-of-sample estimate. We claim that constraint-aware ordering helps reproducibly \emph{on these domains under this protocol}, not that a policy derived this way transfers to unseen ones; the suite-wide null is direct evidence that it would not. A clean test would pre-register the five domains and evaluate on instances withheld from the selection step. Second, all runs come from a single machine and protocol, so absolute coverage is not comparable to published IPC figures, and we lack a clean replicate from which to quantify the coverage resolution of cross-run comparisons; only within-run contrasts should be read quantitatively. Third, the $k$-ordering ceiling projections are extrapolations calibrated on the measured two-ordering oracle, not measurements, and we treat them as such.
