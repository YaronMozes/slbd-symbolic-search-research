...
\begin{equation*}
    \text{SkipCausal}(v_i, v_j) = \mathbb{I}\left( (v_i, v_j) \in E_{\text{causal}} \lor (v_j, v_i) \in E_{\text{causal}} \right)
\end{equation*}
...
\end{equation}
When proposing a candidate swap between positions $p_1$ and $p_2$ (with $p_1 \neq p_2$), the change in objective value $\Delta \mathcal{J}(p_1, p_2)$ is computed incrementally in $\mathcal{O}(|\mathcal{V}|)$ time:
\begin{equation}
\begin{aligned}
    &\ \Delta \mathcal{J}(p_1, p_2) = \sum_{k \neq p_1, p_2} w\left(\pi(k), \pi(p_1)\right) \left[ (k - p_2)^2 - (k - p_1)^2 \right] \\
    &\ + w\left(\pi(k), \pi(p_2)\right) \left[ (k - p_1)^2 - (k - p_2)^2 \right]
\end{aligned}
\label{eq:swap_delta}
\end{equation}
...
Our investigation revealed four methodological insights for automated planning evaluations:
...
\subsubsection{Small-Sample Optimism in Suite-Wide Estimates}
An earlier internal evaluation on a smaller sample (551 instances, 18 domains) suggested the weight-fixed combined ordering strictly dominated stock \textsf{SymK} suite-wide (+3 net coverage). This did not replicate at the full 1{,}847-instance scale (Table~\ref{tab:suitewide_results}), underscoring that suite-wide coverage claims below our measured $\pm 17$-instance noise floor should not be trusted regardless of their apparent direction, reinforcing the case for the pre-registered, domain-targeted protocol used for our confirmed result.

\subsubsection{A Silent Benchmark-Harness Defect}
\label{sec:harness}
Seven IPC domains ship one domain file per instance (e.g.\ \texttt{p01-domain.pddl} for \textit{airport}). Our experiment driver failed to match these naming conventions and fell back to the first domain file in the directory, silently pairing instances with the \emph{wrong} domain. The defect caused 225 instances to fail within ${\sim}0.17\,\text{s}$ in all configurations and -- the diagnostic tell -- three instances to report plan costs \emph{below} the known optimum, because a mis-paired but syntactically valid domain yields a genuinely optimal plan for the wrong task. Because the pairing was identical across configurations, every A/B delta remained intact, but absolute coverage was depressed (stock \textsf{SymK} $1021 \to 1145$ after repair). We re-ran all 230 affected instances under corrected pairing and report only repaired data. We recommend cross-planner optimal-cost agreement as a cheap tripwire: an impossible cost is a far louder signal of harness corruption than a missing solution.
...
% ==================== TABLE 3 ====================
\caption{Suite-wide performance across all 1,847 IPC optimal-STRIPS instances (repaired dataset, Section~\ref{sec:harness}). Time ratios are \textbf{wall-clock relative to stock}, including all selector probe overhead, over instances solved by both the row configuration and stock; intervals are domain-clustered bootstrap 95\% CIs. The selector's search-only ratio, which excludes its probe tax, is 0.992 -- its entire deficit is probe overhead.}
...
\begin{enumerate}\setlength{\itemsep}{1pt}
    ...
    \item \textbf{Cost-Agreement Tripwires:} Cross-planner optimal-cost agreement exposed a silent benchmark-harness defect that missing-solution counts alone would not have revealed (Section~\ref{sec:harness}).
\end{enumerate}
...
\begin{itemize}\setlength{\itemsep}{2pt}
    ...
    \item \textbf{Ceiling Bounds \& Methodological Insights:} Empirical protocols establish a $\pm 17$-instance resolution limit, proving why portfolios and selection wrappers cannot yield suite-wide gains, while highlighting critical traps in hard-instance subsetting, IPC duplicate leakage, and silent harness defects.
\end{itemize}
