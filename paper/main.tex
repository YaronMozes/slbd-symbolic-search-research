...
\author{
  ...
  \begin{minipage}[t]{0.45\textwidth}
    \centering
    \textbf{Kadzelashvily Galit}\\[0.4em]
    \small Faculty of Data and Decision Sciences\\
    \small Technion -- Israel Institute of Technology\\
...
\begin{funfact}[Headline Result: Repeated $+6$ Coverage on the Five Target Domains]
On the five domains it targets, the pre-registered policy gains $+6$ coverage in each of two replicates (64/65 $\to$ 70/71 of 120 instances) and accelerates \textit{woodworking} by 1.55$\times$, with no cost disagreement on any commonly solved instance (domain selection and scope: Section~\ref{sec:limits}).
\end{funfact}
...
Three limitations scope our claims. (i) The five signal domains were identified in the same full-suite runs used for the suite-wide analysis; the pre-registered A/B therefore establishes that the gain on \emph{these} domains is reproducible rather than noise, not that a policy derived this way would transfer to unseen domains. (ii) All tasks ran under the IPC optimal-track limits (1800\,s, 8\,GB, one core), but on a single shared multi-core server; since coverage at a fixed time limit also depends on hardware and load, absolute totals are not directly comparable across papers -- within-run contrasts, which cancel these factors, are the quantitatively meaningful ones. (iii) The $k$-ordering ceiling projections are model extrapolations, not measurements.
...
\begin{itemize}\setlength{\itemsep}{2pt}
    \item \textbf{Reproducible Per-Domain Gain:} A pre-registered, double-replicated A/B policy gains \textbf{$+6$ coverage} in both replicates on its five target domains and accelerates \textit{woodworking} by \textbf{$1.55\times$}, with no cost disagreement. This is same-task reproducibility on selected domains (Section~\ref{sec:limits}), not a general planner advance.
